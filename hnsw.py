...
if __name__ == '__main__':
    ...
    efCons = int(sys.argv[3])
    efSear = int(sys.argv[4])
    k = int(sys.argv[5])

    print(dataset, Maxdeg, efCons, efSear, k, " HNSW")
    ...
    faiss_index = faiss.IndexHNSWFlat(d, Maxdeg)
    ...
    start = time.time()
    faiss_index.hnsw.efConstruction = efCons
    faiss_index.add(xb)
    faiss_index.hnsw.efSearch = efSear
    end = time.time()
    traintime = end - start
    print("time to train is ", traintime)
    ...
    filename = 'results/' + dataset +'-HNSW' + '.csv'
    with open(filename, 'a') as fd:
        fd.write(str(Maxdeg) + ',' + str(efCons) + ',' + str(efSear) + ',' + str(traintime) + ',' + str(qps) + ',' + str(filesz) + ',' + str(recall_score) + ',' + str(k) +'\n')