...
if __name__ == '__main__':
    ...
    pqparam = int(sys.argv[3])
    nprobe = int(sys.argv[4])
    k = int(sys.argv[5])


    print(dataset, ncentroid, pqparam, nprobe, k, " IVFADC")
    ...
    d = xb.shape[1]
    # IVF X, PQ YxZ => IVF has X centroids, PQ has Y partitions and nbits = Z 
    consparam = "IVF"+str(ncentroid)+",PQ"+str(pqparam)+"x8"
    ...
    filename = 'results/' + dataset +'-IVFADC' + '.csv'
    with open(filename, 'a') as fd:
        fd.write(str(traintime) + ',' + str(qps) + ',' + str(memory_in_MB) + ',' + str(recall_score) + ',' + str(ncentroid)+','+ str(pqparam)+','+ str(nprobe)+','+ str(k) +'\n')